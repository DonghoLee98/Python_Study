        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    # # 이중 for 문 중 하위 for문만 탈출, 다음 for문의 초반까지 돌면서 하위 공 내용이 잘못 업데이트 되는 문제가 발생한다.
                    # break

        # for문도 if문처럼 동작할 수 있다
                    break
        else:   # 게임 계속 진행
            continue    # 하위 for문 조건이 맞지 않을 시 continue ( = 상위 for문 계속해서 진행)
        break   # 하위 for문에서 break 걸릴 시 여기로 이동 -> 이중 for문 바로 탈출

        # 이중 for문 탈출; 다시 한 번 더 정리
        # for 상위조건:
        #     상위동작
        #     for 하위조건:
        #         하위동작
